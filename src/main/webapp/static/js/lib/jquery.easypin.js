(function ($) {
    // added a flag to determine to use localStorage or not
    ...
    $.fn.easypin = function (options) {
        ...
        // function callback after easypin initialized  (By Robert)
        ...
        willPinningElements.each(function (i) {
            // ignore the Initialized image --Robert
            if ($(this).hasClass('easypin-target')) {
                loadedImgNum += 1;
                return;
            }
            $(this).css('opacity', 0);
        ...
        // Only remove the saved data at first time  --Robert
        if (localStorage && loadedImgNum === 0) {
            ...
            if (loadedImgNum === total) {
                ...
                willPinningElements.each(function (i) {
                    ...
                    if (imageHeight > 0) {
                        ...
                        // insert easypinId to 'this' value --Robert
                        $.fn.easypin.defaults.easypinId = easypinId;
                ...
                $(parentElement).hover(function () {
                    ...
                    // Fixed bug: added more than one hover mask layer on the image (By Robert)
                    if (!is_open('popup', $(this)) && $(parentElement).children(setClass(hoverClass)).length === 0) {
                        $(this)
                            .prepend(
                    ...
                    // Don't remove hover layer When pop up window shows on page (By Robert)
                    // As when the popup window opened it will not add hover class again
                    // when the mouse moved into image area
                    ...
                    .bind('mousedown', function (e) {
                        ...
                        if (!$(e.target).is(setClass(parentClass)) &&
                            !$(e.target).is(setClass(hoverClass)) &&
                            !$(e.target).is(setClass(pinMapClass)) &&
                            !$(e.target).is(setClass("easypin-target"))
                        ) {
                            e.stopPropagation();
                            return;
                        }
                        ...
                        var absX = markerBorderX.toFixed(3);
                        var absY = markerBorderY + markerHeight;
                ...
                // function callback after easypin initialized  (By Robert)
                if (afterInit && $.isFunction(afterInit)) {
                    afterInit();
                }
            }
        });
    ...
    var createMarker = function (depends) {
        ...
        $(markerContainer).bind('mousedown', function (e) {
            ...
            var clickedMarker = $(e.target).parent().is('.pinParent');

            // Fixed bug: call mouseover event when mouse down in chrome (By Robert)
            ...
            $(parentElement).bind('mousemove', function (e) {
                ...
                else if (liveY + markerHeightHalf > targetImage.height()) {
                    var relY = targetImage.height() - markerHeightHalf;
                }

                if (liveX - markerWidthHalf < 0) {
                    var relX = markerWidthHalf;
                }
                else if (liveX + markerWidthHalf > targetImage.width()) {
                    var relX = targetImage.width() - markerWidthHalf;
                }
        ...
        $(markerContainer).bind('mouseup', function (e) {
            ...
            var realImageWidth = $(parentElement).width();
            var rate = parseFloat(ImgWidth) / realImageWidth;

            dataUpdate(parentId, markerIndex, {coords: {lat: lat * rate, long: long * rate}});
    ...
    $.fn.easypinShow = function (options) {
        ...
        var data = dataObj['pin-image'];
        var tagsArray = [];
        if (data) {
            for (var obj in data) {
                tagsArray.push(data[obj]);
            }
        }
        ...
        try {
            var depends = {
                ...
                allCanvas: this,
                clicked: typeof(options.clicked) !== 'function' ? function () {
                } : options.clicked,
                tagArray: tagsArray,
                anchorPoint: options.anchorPoint || {top: 0.5, left: 0.5}
            };
                    ...
                    $('.easypin-marker:last', pinContainer).click(function (e) {
                        ...
                        if (depends.popover.animate === true) {
                            ...
                            if ($('.easypin-popover', this).css('display') == 'none') {
                                ...
                                // callback start
                                if (depends.clicked) {
                                    var data = depends.tagArray[clickedMarkerIndex - 1];
                                    depends.clicked(ins, data);
                                }
                                // callback end
                            } else {
    ...
    var calculatePinRate = function (data, pinWidth, markerWidth, pinHeight, markerHeight) {
        return {
            left: ((parseInt(data.coords.lat) + 20 - markerWidth) / pinWidth) * 100,
            top: ((parseInt(data.coords.long) - 30) / pinHeight) * 100
        };
    };
    ...
    /**
     * added a new annotation on the image (By Robert)
     * @param coordX
     * @param coordY
     */
    $.fn.easypin.addPin = function (coordX, coordY) {
        var parentElement = setClass($.fn.easypin.defaults.parentClass),
            targetImage = $('img' + setClass('easypin-target'), parentElement),
            imagePositionY = targetImage.offset().top,
            imagePositionX = targetImage.offset().left,
            clickPosX = coordX + imagePositionX,
            clickPosY = coordY + imagePositionY,
            downEvent = $.Event('mousedown', {
                which: 1,
                pageX: clickPosX,
                pageY: clickPosY
            });
        // Added hover class
        $($(parentElement)[0]).mouseover();
        // trigger mouse down event, add a new annotation
        $($(setClass($.fn.easypin.defaults.parentClass))[0]).trigger(downEvent);
    };
    ...
    var createPopup = function (elem, markerContainer) {
        ...
        var modalContext = $('<div/>')
            ...
            .css({
                ...
                'opacity': '0',
                'position': 'absolute',
                'padding': '30px', // STYLEHUB
                '-webkit-box-shadow': '2px 2px 5px 0px rgba(0,0,0,0.75)',// STYLEHUB
                '-moz-box-shadow': '2px 2px 5px 0px rgba(0,0,0,0.75)',// STYLEHUB
                'box-shadow': '2px 2px 5px 0px rgba(0,0,0,0.75)',// STYLEHUB
        ...
        var modalHeight = parseInt($(modalContext).css("height"), 10);
        var modalWidth = parseInt($(modalContext).css("width"), 10);
        ...
        // Added by Robert
        // Moved the modal position to the center
        var modalLeftPosition = (width - modalWidth) * 0.5;
        var modalTopPosition = (height - modalHeight) * 0.5;

        // Fixed bug: If the tag near the top of the photo, the tag popup box won't show all.
        // start-----(By Robert)
        if (modalTopPosition < 0) {
            modalTopPosition = 0;
        }
        ...
        //close dialog modal by click cancel button - STYLEHUB
        $('.easy-cancel-dialog').click(function () {
            closePopup(parentElement);
        });
        ...
        $('.easy-submit', modalContext)
            .click(function () {
                ...
                var realImageWidth = $(parentElement).width();
                var rate = ImgWidth / realImageWidth;
                ...
                var formData = getFormData(modalBody, function (data) {
                    ...
                    data.coords['lat'] = lat * rate;
                    data.coords['long'] = long * rate;
                ...
                // Added new callback function when saved new annotation (By Robert)
                if ($.fn.easypin.defaults.afterDone && typeof($.fn.easypin.defaults.afterDone) === 'function') {
                    $.fn.easypin.defaults.afterDone(formData);
                }
            });
    ...
    var inputInsert = function (parentId, markerIndex, data) {
        ...
        if (typeof(items[parentId]['canvas']) === 'undefined') {
            ...
            // fixed bug: the canvas size is different when edit the pins (Added By Robert)----start
            var canvas = items[parentId]['canvas'];
            var ori_image_width = canvas.width;
            if (data.coords.canvas) {
                rate = parseFloat(ori_image_width) / parseFloat(data.coords.canvas.width);
                data.coords.lat = parseFloat(data.coords.lat) * rate;
                data.coords.long = parseFloat(data.coords.long) * rate;
            }
            // fixed bug: the canvas size is different when edit the pins (Added By Robert)----end
        }
            ...
            try {
                ...
                if (totalPin < 1) {
                    ...
                    if ($.isEmptyObject(items)) {
                        $('input[name="easypin-store"]').remove();
                    }
                }
    ...
    var closePopup = function (parentElement) {
        ...
        // Fixed bug: can't add annotation after closed popup (By Robert)
        // Readd the hover class to the window
        parentElement.children(setClass($.fn.easypin.defaults.hoverClass)).show();
    ...
    var initPin = function (imgIndex, element) {
        ...
        if (sizeof(initData) > 0 && $(element).attr('easypin-init') != 'false') {
            ...
            for (var i in initData[imgIndex]) {
                ...
                var clickPosX = lat * rate;//(lat-imagePositionX);
                var clickPosY = long * rate;//(long-imagePositionY);
                // get marker half size (width/height)
                ...
                if (markerBorderX < -(config.markerWidth / 2)) {
                    markerBorderX = -(config.markerWidth / 2);
                }
                else if (markerBorderX + markerWidthHalf > realImageWidth) {
                    markerBorderX = imageWidth - config.markerWidth;
                }

                if (markerBorderY < -(config.markerHeight / 2)) {
                    markerBorderY = -(config.markerHeight / 2);
                }
                else if (markerBorderY + markerHeightHalf > realImageHeight) {
